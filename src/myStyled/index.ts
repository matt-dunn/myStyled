import React, {
  ComponentType, ReactNode, useContext, useRef,
} from "react";

import { createHash } from "./hash";
import { parseRule, generateClassName, updateSheetRule } from "./utils";
import {
  ClientServerStylesheet, createStylesheet, StyleContext, AnyRules,
} from "./stylesheet";
...
const myStyled = <P>(Component: MyStyledComponent<P>): MyStyled<P, MyStyledTemplate<P>> => (strings, ...args) => {
  ...
  const MyStyled = ({ children, className, ...props }: MyStyledComponentProps) => {
    const prevClassName = useRef<string | undefined>(undefined);

    prevClassName.current = updateRule(prevClassName.current, props, useContext(StyleContext) || globalStylesheet);

    return React.createElement<any>(Component, { ...props, className: [className, prevClassName.current].join(" ") }, children);
  };
